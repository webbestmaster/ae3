    ...
    APP.BB.BattleView = APP.BB.BaseView.extend({
        ...
        markActiveSquare: function(xy) {
            ...
            var view = this,
                ...
                squareEventHandler = util.findIn(
                    view.$el[0],
                    selectors.eventSquares + '[data-xy="x' + x + 'y' + y + '"]'
                );
        ...
        autoSetSquareInfo: function() {
            ...
            $el.find('.js-status-bar-info-square-container')
                .remove()
                .empty();
        ...
        createEventHandlerListener: function(data) {
            // x, y, className
            ...
            node.setAttribute(
                'style',
                pre + 'transform: translate3d(' + x * squareSize + 'px, ' + y * squareSize + 'px, 0);'
            );
        ...
        drawMap: function() {
            ...
            angleTypes.forEach(function(type) {
                _.each(terrains, function(value, xy) {
                    ...
                    // draw 2, 4, 6, 8
                    if (t2) {
                        // up is different type
                        ctx.drawImage(
                            mapTiles['a-' + type + '-2'].img,
                            xSquareSizeX2,
                            ySquareSizeX2,
                            squareSizeX2,
                            squareSize
        ...
        showHouseSmoke: function() {
            var view = this,
                ...
                $smokeWrapper = $('<div/>')
                    .attr('class', 'js-smoke-wrapper smoke-wrapper')
                    .attr('style', $eventHandleWrapper.attr('style'));
        ...
        appendBuilding: function(building) {
            ...
            $node
                ...
                .attr('data-x', x)
                .attr('data-y', y)
                .attr('data-type', building.type);

            $node
                .addClass('building')
                .addClass('js-building')
                .addClass('square');
        ...
        addSmokeToBuilding: function(building) {
            var x = building.x,
                ...
                $smokeContainer = $(
                    '<div class="square js-square"><div class="building-smoke-mover"><div class="building-smoke">&nbsp;</div></div></div>'
                );
        ...
        onRedrawMapFromMover: function(data) {
            ...
            mover.set('currentContainerXY', {
                // fix if user up two finger simultaneously
        ...
        detectPress: function() {
            ...
            if (Math.abs(downEvent.x - moveEvent.x) + Math.abs(downEvent.y - moveEvent.y) > 7) {
                // detect press event without move
                return;
            }
        ...
        showAttack: function(data) {
            ...
            $attackNode.css(
                pre + 'transform',
                'translate3d(' + from.x * squareSize + 'px, ' + from.y * squareSize + 'px, 0)'
            );

            $attackNode.one(transitionEnd, function() {
                //$(this).remove();
                view.$el
                    .find('.js-attack-square')
                    .remove()
                    .empty();
            ...
            setTimeout(function() {
                // todo: try to do transitionEnd without this hack
                $attackNode.css(
                    pre + 'transform',
                    'translate3d(' + to.x * squareSize + 'px, ' + to.y * squareSize + 'px, 0)'
                );
            }, 50);
        ...
        showDifferentUnitHealth: function(data) {
            var view = this,
                ...
                $deltaHealth = $(
                    '<div class="js-delta-unit-health delta-unit-health"><div class="js-delta-unit-health-sign delta-unit-health-sign">&nbsp;</div><div class="js-delta-unit-health-ten delta-unit-health-ten">&nbsp;</div><div class="js-delta-unit-health-one delta-unit-health-one">&nbsp;</div></div>'
                ),
                animationEnd = view.info.get('animationEnd', true);
        ...
        setUnitHealth: function(data) {
            ...
            if (one === 'none') {
                ...
                if ($healthOne.length) {
                    ...
                    $unitWrapper.append(
                        '<div class="js-unit-health-one unit-health-one number-1-' + one + '">&nbsp;</div>'
                    );
                }
            }
            ...
            if (ten === 'none') {
                ...
                if ($healthTen.length) {
                    ...
                    $unitWrapper.append(
                        '<div class="js-unit-health-ten unit-health-ten number-1-' + ten + '">&nbsp;</div>'
                    );
                }
            }
        },
        ...
        setUnitLevel: function(data) {
            ...
            if (!data.doNotShowLevelUp) {
                // when commander was killed and was buy in unit store
                view.showLevelUp({
                    unit: unit
                });
            }
