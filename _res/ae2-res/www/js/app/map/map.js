...
(function(win, doc) {
    ...
    win.APP.map = {
        ...
        recountProgress: function() {
            this.progress += 1;
            doc.querySelector('.js-progress-bar').style.width =
                Math.round((this.progress / this.progressLength) * 100) + '%';
        },

        preloadData: function() {
            this.progressLength =
                win.APP.allImages.length +
                _.keys(win.APP.mapTiles).length +
                _.keys(win.APP.mapTilesPreview).length +
                _.keys(win.APP.maps).length;
        ...
        db: {
            ...
            init: function() {
                ...
                db.transaction(function(tx) {
                    ...
                    function createMissionMapTables() {
                        tx.executeSql(
                            'CREATE TABLE IF NOT EXISTS ' +
                                dbMaster.missionMaps +
                                ' (jsMapKey TEXT, info TEXT, map TEXT)',
                            [],
                            function() {
                    ...
                    if (currentMapVersion > previousMapVersion) {
                        // remove all maps
                        info.set('mapPackVersion', currentMapVersion);
                        tx.executeSql(
                            'DROP TABLE IF EXISTS ' + dbMaster.skirmishMaps,
                    ...
                    tx.executeSql(
                        'CREATE TABLE IF NOT EXISTS ' + dbMaster.savedGame + ' (date TEXT, name TEXT, game TEXT)',
                        [],
                        null,
                        null
                    );
                    tx.executeSql(
                        'CREATE TABLE IF NOT EXISTS ' + dbMaster.userMap + ' (jsMapKey TEXT, info TEXT, map TEXT)',
                        [],
                        null,
                        null
                    );
            ...
            compareMap: function(oldMap, newMap, jsMapKey) {
                var maps = win.APP.maps,
                    ...
                    savedProperties = [
                        'isOpen',
                        'isDone',
                        'isDoneByDifficult_easy',
                        'isDoneByDifficult_normal',
                        'isDoneByDifficult_hard'
                    ];
            ...
            insertMap: function(map, jsMapKey) {
                // map
                ...
                db.transaction(function(tx) {
                    tx.executeSql(
                        'INSERT INTO ' + map.type + ' (jsMapKey, info, map) values(?, ?, ?)',
                        [jsMapKey, JSON.stringify(info), JSON.stringify(map)],
                        function() {
                            deferred.resolve();
                        },
                        null
            ...
            removeMap: function(data) {
                ...
                db.transaction(function(tx) {
                    tx.executeSql(
                        'DELETE FROM ' + type + ' WHERE jsMapKey = ?',
                        [jsMapKey],
                        function() {
                            deferred.resolve();
                        },
            ...
            removeSave: function(name) {
                ...
                db.transaction(function(tx) {
                    tx.executeSql(
                        'DELETE FROM ' + dbMaster.savedGame + ' WHERE name = ?',
                        [name],
                        function() {
                            deferred.resolve();
                        },
            ...
            getMapInfo: function(data) {
                ...
                db.transaction(function(tx) {
                    tx.executeSql('SELECT * FROM ' + data.type + ' WHERE jsMapKey=?', [data.jsMapKey], function(
                        tx,
                        results
                    ) {
            ...
            saveGame: function(date, name, data) {
                ...
                dbMaster.removeSave(name).then(function() {
                    db.transaction(function(tx) {
                        tx.executeSql(
                            'INSERT INTO ' + dbMaster.savedGame + ' (date, name, game) values(?, ?, ?)',
                            [date, name, JSON.stringify(data)],
                            function() {
                                deferred.resolve();
                            },
                            null
            ...
            getSavedGames: function() {
                ...
                db.transaction(function(tx) {
                    tx.executeSql('SELECT * FROM ' + dbMaster.savedGame + ' ORDER BY date DESC', [], function(
                        tx,
                        results
                    ) {
                        var i, len, row;
            ...
            getSavedGame: function(gameName) {
                ...
                db.transaction(function(tx) {
                    tx.executeSql('SELECT * FROM ' + dbMaster.savedGame + ' WHERE name=?', [gameName], function(
                        tx,
                        results
                    ) {
