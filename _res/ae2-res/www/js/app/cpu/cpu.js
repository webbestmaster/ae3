...
(function(win) {
    ...
    Cpu.prototype = {
        ...
        runScenario: function(scenario) {
            ...
            view.showAvailableActions(unit.getAvailableActions())
                // move if needed
                .then(function() {
            ...
            _.each(['getBuilding', 'raiseSkeleton', 'attack', 'fixBuilding', 'move'], function(scenarioType) {
                ...
                switch (scenarioType) {
                        ...
                        _.each(filteredScenarios, function(scenario) {
                            ...
                            if (availableReceiveDamage) {
                                scenario.changeBy(
                                    ...
                                    placeArmor +
                                        onHealthUpBuilding +
                                        availableReceiveDamage * rates.q.availableReceiveDamage
                                );
                            }
                        });
                            ...
                            if (availableReceiveDamage) {
                                scenario.changeBy(
                                    ...
                                    placeArmor +
                                        onHealthUpBuilding +
                                        availableReceiveDamage * rates.q.availableReceiveDamage -
                                        scenario.get('availableResponseDamage')
                                );
                            }
                        });
                    ...
                    case 'move':
                        ...
                        if (
                            gameDifficult !== 'easy' &&
                            (model.playerHasCastle(player) ||
                                model.playerHasCommander(player) ||
                                gameDifficult === 'hard' ||
                                (gameDifficult === 'normal' && Math.random() > 0.5))
                        ) {
                            // if mission has no buildings
                            ...
                            if (moveHp40Plus.length) {
                                // find scenario for normal move

                                filteredScenarios = moveHp40Plus.sort(function(sc1, sc2) {
                                    var unit1 = sc1.get('unit'),
                                        unit2 = sc2.get('unit'),
                                        length1 =
                                            (unit1.get('listOccupyBuilding') || []).length * rates.q.listOccupyBuilding,
                                        length2 =
                                            (unit2.get('listOccupyBuilding') || []).length * rates.q.listOccupyBuilding;
                                ...
                                if (nullAvailableReceiveDamage.length) {
                                    // if nullAvailableReceiveDamage is exist
                                    ...
                                    filteredScenarios = filteredScenarios.sort(function(sc1, sc2) {
                                        return (
                                            sc1.get('dataByPosition').availableReceiveDamageWithPath -
                                            sc2.get('dataByPosition').availableReceiveDamageWithPath
                                        );
                                    });
                                }
                            }
        ...
        getUnitScenariosByXY: function(data) {
            ...
            _.each(actionList, function(action) {
                var scenario, unitsUnderAttack, buildingToGet, buildingToFix;
                ...
                switch (action) {
                    ...
                    case 'attack':
                        ...
                        if (!(unit.get('canNotActionAfterMove') && (unitX !== x || unitY !== y))) {
                            // detect moved catapult
        ...
        detectScenarioCanCoverBuilding: function(scenarios) {
            ...
            function getXyBuildingToEnemyGet(dataArg) {
                ...
                _.each(enemyUnits, function(enemy) {
                    ...
                    if (!listOccupyBuilding.length) {
                        // detect units who can get buildings
                        return;
                    }
                    ...
                    if (data.blackWholes) {
                        if (cachedAvailableMoveMap) {
                            ...
                            _.each(cachedAvailableMoveMap, function(xy) {
                                return (
                                    isBlackWholeInCachedAvailableMoveMap ||
                                    (isBlackWholeInCachedAvailableMoveMap = _.find(blackWholesXYs, xy))
                                );
                            });
                            availableMoveMap = isBlackWholeInCachedAvailableMoveMap
                                ? enemy.getAvailablePathFull(data)
                    ...
                    _.each(availableMoveMap, function(moveXY) {
                        ...
                        if (listOccupyBuilding.indexOf(building.type) === -1) {
                            // if building is not in occupy list
                            return;
                        }

                        if (building.teamNumber === enemyTeamNumber) {
                            // enemy not reason get own building twice
                            return;
                        }
            ...
            _.each(teamUnits, function(unit) {
                ...
                _.each(unitScenarios, function(unitScenario) {
                    ...
                    unitScenario.set(
                        'coverBuildingCount',
                        enemyUnitsGetAvailableBuildingsFull.length - enemyUnitsGetAvailableBuildingsByScenario.length
                    );
                });
            });
        },
        ...
        insertDataByPosition: function(scenario) {
            ...
            if (building) {
                ...
                if (building.teamNumber === null) {
                    ...
                    if (building.teamNumber !== unit.get('teamNumber')) {
                        // detect enemy building
        ...
        rateMove: function(data) {
            ...
            if (!wantedBuildings.length || (!hasCastle && !hasCommander)) {
                // if mission or no needed buildings

                _.each(enemyUnits, function(enemy) {
                    pathToBuildingLength = Math.min(
                        pathToBuildingLength,
                        util.getPathSize({x: enemy.get('x'), y: enemy.get('y')}, xy)
                    );
                });
            } else {
                _.each(wantedBuildings, function(building) {
                    ...
                    if (
                        teamUnit &&
                        teamUnit.get('teamNumber') === unitTeamNumber &&
                        _.contains(teamUnit.get('listOccupyBuilding'), building.type)
                    ) {
                        return;
                    }
        ...
        rateAttack: function(data) {
            ...
            if (availableGivenDamage >= enemyHealth) {
                ...
                if (availableResponseDamage < unit.get('health') - 20) {
                    // detect: unit will be alive after attack
