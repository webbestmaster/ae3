...
(function(win) {
    ...
    win.APP.BB.Unit.BaseUnitModel = Backbone.Model.extend({
        ...
        autoSetActiveState: function() {
            ...
            isActive =
                isActive ||
                (actions.availablePathViewWithoutTeamUnit && actions.availablePathViewWithoutTeamUnit.length);
        ...
        getAvailablePathWithTeamUnit: function(data) {
            ...
            _.each(units, function(unit) {
                if (unit.get('teamNumber') === unitTeamNumber) {
                    // unit can move around alias units
                    return;
                }
        ...
        getAttackToUnit: function(enemy, dataArg) {
            var data = dataArg || {},
                ...
                unitFlowAtkBonus =
                    unitMoveType === 'flow' && unitTerrain.terrainType === 'water' ? unitMaster.bonusAtkByWater : 0,
                enemyMoveType = enemy.get('moveType'),
                enemyType = enemy.get('type'),
                unitAtkBonusAgainstFly =
                    enemyMoveType === 'fly' && unit.get('bonusAtkAgainstFly') ? unit.get('bonusAtkAgainstFly') : 0,
                unitAtkBonusAgainstSkeleton =
                    enemyType === 'skeleton' && unit.get('bonusAtkAgainstSkeleton')
                        ? unit.get('bonusAtkAgainstSkeleton')
                        : 0,
                ...
                enemyFlowDefBonus =
                    enemyMoveType === 'flow' && enemyTerrain.terrainType === 'water' ? unitMaster.bonusDefByWater : 0,
                unitStartAtk = 0,
            ...
            atk =
                (unitStartAtk +
                    ...
                    unitAtkBonusAgainstFly +
                    unitAtkBonusAgainstSkeleton -
                    unitAtkReduceByPoison +
                    unitAktBonusByWispAura) *
                    unitQ -
                (enemyDefBonusByLevel + enemyFlowDefBonus - enemyDefReduceByPoison + enemyDef + enemyDefTerrain) *
                    unitQ; // * enemyQ;
        ...
        moveTo: function(data) {
            ...
            view.moveUnitTo(data).done(function() {
                unit.set('x', x);
                unit.set('y', y);
