    ...
    win.APP.BB.Unit.CatapultModel = win.APP.BB.Unit.BaseUnitModel.extend({
        ...
        getUnitsUnderAttack: function() {
            ...
            _.each(buildings, function(building) {
                ...
                if (!_.find(availableAttackMap, xy)) {
                    // do not add extra buildings
                    return;
                }

                if (!_.contains(availableStates, 'destroyed')) {
                    // do not add without 'destroyed' state
                    return;
                }
                ...
                if (unitTeamNumber === building.teamNumber) {
                    // do not destroy team building
                    return;
                }

                if (null === building.teamNumber) {
                    // do not destroy no man's building
                    return;
                }

                if (_.find(underAttackXYs, xy)) {
                    // do not add the same XY twice
                    return;
                }

                if (unitOnXY && unitOnXY.get('teamNumber') === unit.get('teamNumber')) {
                    // do not add xy if team unit on building
                    return;
                }
