    ...
    win.APP.BB.BattleModel = Backbone.Model.extend({
        ...
        appendUnits: function() {
            ...
            _.each(mapUnits, function(unit) {
                ...
                if (!player) {
                    // detect player.type === 'none' was deleted from players
                    return;
                }
            ...
            if (unit) {
                ...
                if (unit.get('isActive')) {
                    if (unitOwnerId === activePlayer.id) {
                        ...
                        if (building && buildingData[building.type].canBeStore) {
                            // try to open store
                    ...
                    if (
                        unitOwnerId === activePlayer.id &&
                        building &&
                        buildingData[building.type].canBeStore &&
                        building.ownerId === activePlayer.id
                    ) {
                        // try to open store
                        model.openStore(xy);
                    }
        ...
        getTerrainByXY: function(xy) {
            ...
            if (!terrainName) {
                // if terrain with xy is not exist -> return false
                return false;
            }
        ...
        checkPlayerDefeat: function() {
            if (!/skirmish|userMap/.test(this.get('map').type)) {
                // detect skirmish and user map
                return;
            }
            ...
            if (teamOfLooser.length) {
                ...
                if (_.where(players, {type: 'cpu'}).length === players.length) {
                    // cpu only
                    //win.APP.bb.battleData.isEndGame = 'yes';
                    model.set('isEndGame', true);
                    view.showPopup({
                        ...
                        onHide: {
                            // can be disabled from popup view
                            fn: 'backTo',
        ...
        autoSave: function() {
            ...
            dbMaster.saveGame(saveDate, saveName, gameData).then(function() {
