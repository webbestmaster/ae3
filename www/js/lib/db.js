...
export default class DB {
    ...
    update(tableName, newData, selectorKey, selectorValue) {
        const fieldList = Object.keys(newData);

        if (!fieldList.length) {
            return Promise.resolve();
        }

        const setPart = fieldList.map(field => field + '=?').join(', ');
        const values = fieldList.map(field => newData[field]);

        values.push(selectorValue);
        ...
        return new Promise((resolve, reject) =>
            this
                ...
                .transaction(tx =>
                    tx.executeSql(
                        `UPDATE ${tableName} SET ${setPart} WHERE ${selectorKey}=?`,
                        values,
                        resolve,
                        reject
                    )
